...
def build_sidebar():
    ...
    with st.sidebar:
        ...
        # --- TIPE FILE DIPERBARUI UNTUK SEMUA UPLOADER ---

        # Tipe file yang diizinkan untuk data operasional
        tipe_file_standar = [
            "xlsx",
            "csv",
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",  # MIME type untuk .xlsx
            "text/csv",  # MIME type untuk .csv
        ]

        gmv_file = st.file_uploader(
            "1. Upload Laporan GMV (Operasional)",
            type=tipe_file_standar,  # <-- Diperbarui
        )
    ...
    return gmv_file, cogs_file, waiter_file, ulasan_file
...
def main():
    ...
    # 1. Gambar Sidebar dan dapatkan file (UBAH BARIS INI)
    gmv_file, cogs_file, waiter_file, ulasan_file = build_sidebar()

    # 2. Muat data (TAMBAHKAN BARIS INI)
    data_gmv = load_data_gmv(gmv_file)
    data_cogs = load_cogs_data(cogs_file)
    data_waiter = load_data_waiter(waiter_file)
    data_ulasan = load_data_ulasan(ulasan_file)  # <-- BARIS BARU
    ...
    # --- ⚡️ PERBAIKAN JUDUL TAB DI SINI ⚡️ ---
    tab_titles = [
        "📊 KPI Penjualan",
        "💰 COGS & Profit",
        "🧑‍🍳 SDM & Waktu",
