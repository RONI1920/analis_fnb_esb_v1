...
def load_data_gmv(uploaded_file, use_db=False):
    ...
    if use_db:
        with st.spinner("Memuat data GMV dari database..."):
            ...
            if df is None:
                # Jangan tampilkan error, cukup info. Welcome screen akan muncul.
                print("Database GMV kosong atau tidak ada.")
                return None, None, None, None

            # Jika berhasil, kembalikan data DB
    ...
    # Ini adalah kasus "data kosong" yang Anda sebutkan
    return None, None, None, None
...
def build_tab11_musiman(df_gmv, df_kalender):
    ...
    st.info("Grafik batang menampilkan **TOTAL PENDAPATAN**, sedangkan Insight teks menganalisis **RATA-RATA HARIAN**.")
    ...
    if df_summary is not None and not df_summary.empty:
        ...
        df_summary['Label_Total'] = df_summary['Total_Omzet'].fillna(0).apply(format_rupiah)
        df_summary['Label_Rata'] = df_summary['Rata_Rata_Omzet'].fillna(0).apply(format_rupiah)
        ...
        if 'Nama_Event' in df_raw.columns:
            df_libur = df_raw[df_raw['Kategori_Hari'].str.contains('Libur', na=False)].copy()
            
            if not df_libur.empty:
                df_libur['Tanggal_Str'] = df_libur['Tanggal'].dt.strftime('%d %B %Y')
                df_libur['Omzet'] = df_libur['Total After Bill Discount'].apply(format_rupiah)
                
                # Urutkan dulu baru pilih kolom
                df_libur_sorted = df_libur.sort_values('Tanggal', ascending=True)
                tabel_show = df_libur_sorted[['Tanggal_Str', 'Nama_Event', 'Kategori_Hari', 'Omzet']]
                
                st.dataframe(
                    tabel_show.rename(columns={
                        'Tanggal_Str': 'Tanggal', 
        ...
        with st.expander("Klik untuk melihat grafik sebaran detail", expanded=True):
            try:
                df_raw['Hover_Rupiah'] = df_raw['Total After Bill Discount'].apply(format_rupiah)
            ...
            fig_box.update_layout(showlegend=False, yaxis_title="Omzet Harian", yaxis=dict(tickprefix="Rp "))
        ...
        st.write("*Insight di bawah ini dihitung berdasarkan Rata-rata Penjualan per Hari (Apple-to-Apple).*")
        ...
        with col_ins1:
            ...
            if avg_wd_libur > 0 and avg_wd_biasa > 0:
                ...
                st.info("Belum ada data 'Tanggal Merah di Hari Kerja' pada periode yang dipilih.")
        ...
        col_c.metric("Senin-Jumat (LIBUR)", format_rupiah(avg_wd_libur), delta="vs Biasa", delta_color="normal")
...
def build_welcome_screen():
    ...
    * **✨ Menganalisa :**  Pola keterikatan antara Penjualan dengan Hari Libur Nasional, WeekEnd Libur dan WeekDay Libur.
    """
    )
    st.markdown("---")
...
def main():
    ...
    # Jika tidak ada data, TAMPILKAN HANYA Welcome Screen dan BERHENTI.
    if all_data_is_missing:
        build_welcome_screen()  # Panggil fungsi selamat datang
        build_footer()  # Tampilkan footer di sini
        st.stop()  # <-- KEMBALIKAN 'st.stop()' DI SINI

    # --- OPSI B: ADA DATA (Tampilkan Dashboard Lengkap) ---
    # Jika kita sampai di sini, artinya SETIDAKNYA SATU file data telah dimuat.

    # --- 6.A. Tampilkan Header Dinamis ---
    if data_gmv is not None and file_company is not None:
        if use_db == False and file_company != "DB_MODE":
            ...
            st.warning(
                "Tidak ada data GMV yang ditemukan untuk filter/periode yang Anda pilih. Sesuaikan filter Anda."
            )
        else:
            st.title("Dashboard Analisis Data F&B")

    # OPSI 2: Data GMV tidak ada, TAPI data lain ADA (tampilkan info box)
    elif not all_data_is_missing and data_gmv is None:
        st.title("Dashboard Analisis Data F&B")
        st.info(
            "Mode analisis fleksibel. Data GMV tidak dimuat, beberapa filter/header mungkin nonaktif."
        )
    ...
    page_options = [
        ...
        "💡 Rekomendasi",
        "💸 Analisis Promo",
        "✨ Analisis Musiman Tahunan",  # <-- TAMBAHAN BARU
    ]
    ...
    if page == "📊 Penjualan (GMV)":
        ...
        build_tab11_musiman(filtered_gmv, data_kalender)  # <-- Ini baris baru Anda
