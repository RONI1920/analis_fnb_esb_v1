...
def get_menu_performance(df):
    ...
    filter_regex = r"ADD[ -]?ON|ADDITIONAL"
...
def load_data_gmv(uploaded_file, use_db=False):
    ...
    if use_db and uploaded_file is None:
        with st.spinner("Memuat data GMV dari database..."):
            ...
            return df, "Data dari Database", "Semua Periode (DB)", "Semua Cabang (DB)"
...
def build_global_filters(data_gmv, data_cogs, data_waiter, data_purchase):
    ...
    # ... (Blok penentuan master tanggal - tidak ada perubahan) ...
    ...
    if filter_source_file:
        ...
        # #############################################################
        # --- MODIFIKASI: TAMBAHKAN FILTER BRANCH (CABANG) ---
        selected_branches = []
        # Kita cek data_gmv (sumber utama), dan kolom "Branch"
        # Kolom "Branch" sudah di-.title() oleh load_data_gmv
        if data_gmv is not None and "Branch" in data_gmv.columns:
            ...
            # 'filtered_gmv' sekarang hanya berisi cabang yang dipilih
        ...
        # --- MODIFIKASI: Terapkan filter tanggal ke 'filtered_gmv' ---
        # (Bukan 'data_gmv' lagi, agar filter bertumpuk)
        if filter_type == "Harian":
            ...
            if filtered_gmv is not None:  # <-- Ubah 'data_gmv' jadi 'filtered_gmv'
                filtered_gmv = filtered_gmv[  # <-- Ubah 'filtered_gmv' lagi
        ...
        # 'filtered_gmv' (hasil filter cabang) akan diteruskan ke tab
...
def main():
    ...
    # 2. Muat data
    # --- MODIFIKASI BLOK 2: Tangkap return values baru ---
    ...
    # --- MODIFIKASI BLOK 2.5: Tampilkan Header Info ---
    if company:  # Jika data_gmv berhasil dimuat
        # Tampilkan nama perusahaan dan cabang dari header
        st.subheader(f"{company}")
        # Tampilkan periode dari header
