...
import os
import sqlite3  # <-- INI HILANG DARI KODE ANDA
...
# --- FUNGSI-FUNGSI INI HILANG DARI KODE ANDA ---
...
def build_sidebar():
    ...
    with st.sidebar:
        ...
        # --- Uploader 4: Ulasan (DIMODIFIKASI) ---
        ulasan_file = st.file_uploader(
            "4. Upload Laporan Ulasan Pelanggan",
            type=tipe_file_standar,
            key="uploader_ulasan",  # <-- Tambahkan key unik
            on_change=uncheck_db_on_upload,  # <-- Tambahkan callback
        )
    ...
    return gmv_file, cogs_file, waiter_file, ulasan_file, st.session_state.use_db
...
def build_global_filters(data_gmv, data_cogs, data_waiter):
    ...
    if filter_source_file:
        ...
        # PERBAIKAN: Menambahkan 'data_ulasan' tidak diperlukan di sini
        # karena Tab 7 tidak menggunakan filter global.
        ...
        if not filtered_gmv.empty:
            ...
            if "Menu Category" in filtered_gmv.columns and not menu_sales_cat_df.empty:
                with st.expander(
                    ...
                    # 1. Buat data untuk Grafik Kategori
                    data_kategori = (
...
def build_tab4_comparison(data_gmv, data_cogs, data_waiter):
    ...
    # --- ⚡️ PERBAIKAN TATA LETAK DI SINI ⚡️ ---
...
# #################################################################
# --- BAGIAN 3.7: FUNGSI PEMBANGUN UI (TAB 7 - ANALISIS PELANGGAN) ---
# #################################################################
...
def build_tab7_pelanggan(data_ulasan):
    ...
    # --- 2. Tampilkan Distribusi Rating ---
    st.subheader("Grafik Distribusi Rating")

    # --- ⚡️ PERBAIKAN ADA DI SINI ⚡️ ---
    # Nama kolom di pandas modern adalah NAMA_SERIES_ASLI ('Rating_Clean') dan 'count'
    # Bukan 'index' dan 'count'.
    rating_counts = (
    ...
    # --- 💡 PERBAIKAN LABEL GRAFIK (SESUAI PERMINTAAN) 💡 ---
    # Mengatur label sumbu-X (angka rating) agar "berdiri" lurus (horizontal)
    # dengan sudut 0 derajat agar mudah dibaca.
    ...
    # --- 3. Analisis Kualitatif / Kata Kunci ---
    st.subheader("💭 Analisis Topik Kualitatif")

    # Definisikan kata kunci Anda di sini
    POSITIVE_KEYWORDS = {
    ...
    # Fungsi helper untuk menghitung keyword
    def hitung_keyword(df_ulasan, keyword_dict):
        ...
        # Ubah ke dataframe untuk di-chart
        return (
    ...
    with col_pos:
        ...
        # (Pastikan fungsi 'create_horizontal_bar_chart' sudah di-import atau didefinisikan)
    ...
    # --- 4. Tampilkan Data Ulasan Mentah (Dapat Difilter) ---
    ...
    # Menampilkan dataframe tidak diperlukan jika Anda sudah menampilkannya di expander
    # st.dataframe(
    #     filtered_reviews[["Nama", "Rating", "Ulasan"]], use_container_width=True
    # )

    # Tampilkan sebagai expander agar lebih rapi
    st.write(f"Menampilkan {len(filtered_reviews)} dari {total_ulasan} ulasan:")
    with st.container(height=400):  # Container dengan tinggi tetap
...
def main():
    ...
    # --- BARIS BARU: Inisialisasi DB ---
    init_db()

    st.title("📈 Data Driven Specialyst FnB Analyst")

    # 1. Gambar Sidebar
    gmv_file, cogs_file, waiter_file, ulasan_file, use_db = build_sidebar()

    # 2. Muat data (Panggilan PERTAMA dan SATU-SATUNYA)
    ...
    if "save_waiter_flag" in st.session_state and st.session_state.save_waiter_flag:
        ...
        st.session_state.save_waiter_flag = False  # Setel ulang bendera
    ...
    if "save_ulasan_flag" in st.session_state and st.session_state.save_ulasan_flag:
        ...
        st.session_state.save_ulasan_flag = False  # Setel ulang bendera
    # --- AKHIR BLOK BARU ---

    # 3. Gambar Filter Global (Tidak berubah)
    filtered_gmv, filtered_cogs, filtered_waiter = build_global_filters(
        data_gmv, data_cogs, data_waiter
    )

    # --- (Sisa fungsi main Anda TIDAK BERUBAH SAMA SEKALI) ---
    tab_titles = [
        "📊 KPI Penjualan",
        "💰 COGS & Profit",
        "🧑‍🍳 SDM & Waktu",
