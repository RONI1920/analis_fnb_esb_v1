...
def get_visit_purpose_analysis(df):
    ...
    sales_by_visit = (
        df.groupby("Visit Purpose")["Total After Bill Discount"]
        .sum()
        .sort_values(ascending=False)
    )
    return sales_by_visit.reset_index()
...
def get_operational_kpi(df):
    ...
    peak_hours = (
        df.groupby("Hour")["Bill Number"].nunique().sort_values(ascending=False)
    )
    ...
    df["Day Name"] = df["Sales Date In"].dt.day_name().map(day_map)
    peak_days_of_week = df.groupby("Day Name")["Bill Number"].nunique().reset_index()
...
def create_horizontal_bar_chart(data, x_col, y_col, x_title, y_title, sort_order="-x"):
    ...
    chart = (
        ...
        .encode(
            x=alt.X(
                f"{x_col}:Q", title=x_title, axis=alt.Axis(format="~s")
            ),  # Format axis 's' (misal 10k, 1M)
            y=alt.Y(f"{y_col}:N", title=y_title, sort=sort_order),
            color=alt.Color(
                f"{y_col}:N", title=y_title, legend=None
            ),  # Warna berdasarkan Y-axis
            tooltip=[
                alt.Tooltip(y_col, title=y_title),
                alt.Tooltip(x_col, title=x_title, format=",.0f"),  # Tooltip pakai koma
            ],
        )
        .interactive()
    )
...
def create_vertical_bar_chart(
    ...
    chart = (
        ...
        .encode(
            x=alt.X(
                f"{x_col}:{x_type}", title=x_title, sort=sort_order
            ),  # Menambahkan sort order
            y=alt.Y(f"{y_col}:Q", title=y_title),
            color=alt.Color(
                f"{x_col}:{x_type}", title=x_title, legend=None
            ),  # Warna berdasarkan X-axis
            tooltip=[
                alt.Tooltip(x_col, title=x_title),
                alt.Tooltip(y_col, title=y_title, format=",.0f"),  # Tooltip pakai koma
            ],
        )
        .interactive()
    )
...
if uploaded_file is not None:
    ...
    if data is not None:
        ...
        # BARU: Menggunakan format_rupiah() yang memiliki titik
        col1.metric(
        ...
        col3.metric("🧾 Total Transaksi", f"{kpi['Total Transaksi']} tagihan")
        ...
        # BARU: Format angka metrik dengan pemisah ribuan
        col5.metric("📦 Total Item Terjual", f"{kpi['Total Item Terjual']:,.0f} unit")
        col6.metric(
        ...
        st.info(
            "ℹ️ Menu 'PACKAGE', 'ADDITIONAL', dan 'ADD-ONS' telah disaring dari analisis performa menu."
        )
        ...
        with col9:
            ...
            # Format Qty sebagai angka bulat
            st.dataframe(top_selling.set_index("Menu").style.format({"Qty": "{:.0f}"}))
        ...
        with col13:
            ...
            # Format Qty sebagai angka bulat
            st.dataframe(
                bottom_selling.set_index("Menu").style.format({"Qty": "{:.0f}"})
            )
