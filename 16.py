...
def init_db():
    ...
    try:
        ...
        pass  # Biarkan silent jika gagal, Streamlit akan menangani jika datanya kosong
...
def load_data_gmv(uploaded_file, use_db=False):
    ...
    VERSI BARU: Bisa membaca header dari .xlsx dan .csv
    """

    # --- BLOK 1: Logika Pemuatan DB ---
    if use_db and uploaded_file is None:
        with st.spinner("Memuat data GMV dari database..."):
            ...
            # Ambil Branch, Company, Period dari baris pertama (satu entry per baris)
            company_name = df["Company"].iloc[0] if "Company" in df.columns else "N/A"
            period_str = df["Period"].iloc[0] if "Period" in df.columns else "N/A"
            branch_name_header = (
                df["Branch"].iloc[0] if "Branch" in df.columns else "N/A"
            )

            return df, company_name, period_str, branch_name_header
    ...
    try:
        if uploaded_file.name.endswith(".xlsx"):
            # Baca Header dari Excel
            df_header = pd.read_excel(
                uploaded_file, header=None, nrows=7, usecols=[0, 1], engine="openpyxl"
            )

            # Baris 2, Kolom A (index 1, 0) -> Nama Perusahaan
            company_name = str(df_header.iloc[1, 0])
            # Baris 5, Kolom B (index 4, 1) -> Periode
            period_str = str(df_header.iloc[4, 1])
            # Baris 6, Kolom B (index 5, 1) -> Nama Cabang di Header
            ...
            uploaded_file.seek(0)  # Reset pointer lagi sebelum baca data utama
        ...
        if uploaded_file.name.endswith(".csv"):
            st.error(
                "Pastikan header file CSV ada di baris 10 dan data headernya (Baris 2,5,6) ada di kolom B."
            )
        else:
    ...
    # Tambahkan kolom header ke data sebelum disimpan ke DB (untuk fungsi load dari DB)
    df_data["Company"] = company_name
    df_data["Period"] = period_str
    df_data["Branch"] = (
        branch_name_header  # Kolom Branch ini yang akan digunakan untuk filter
    )
...
def build_tab9_rekomendasi(filtered_gmv):
    ...
    # --- Batas Harga & Filter Kategori DIHAPUS ---
...
def main():
    ...
    # --- 6. MEMBUAT NAVIGASI (MENGGANTIKAN st.tabs) ---

    # Buat daftar opsi halaman
    page_options = [
        "📊 Penjualan (GMV)",
        "💰 COGS & Profit",
        "🧑‍🍳 SDM & Waktu",
    ...
    # ... (Kode JavaScript Anda di bagian akhir tetap sama) ...
    st.markdown(
        """
        <script>
