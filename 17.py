...
def main():
    ...
    # --- KONFIGURASI HALAMAN ---
    # (Anda sudah memilikinya di atas, jadi kita lewati di sini)
    ...
    # #############################################################
    # --- 1. INISIALISASI SESSION STATE (HARUS DI ATAS) ---
    # #############################################################

    # Inisialisasi 'flag' (Pemicu)
    ...
    # #############################################################
    # --- 3. PEMUATAN DATA (dari File atau DB) ---
    # #############################################################
    # Kita hanya butuh 1 variabel untuk 'header_info'
    ...
    # #############################################################
    # --- 4. LOGIKA PENYIMPANAN DATA (VERSI FINAL) ---
    # #############################################################

    # Buat 1 fungsi helper untuk membersihkan cache dan me-refresh
    def clear_db_cache_and_rerun():
        load_dataframe_from_db.clear()  # HANYA HAPUS CACHE FUNGSI DB
    ...
    # #############################################################
    # --- 5. FILTER GLOBAL (HARUS SEBELUM HEADER) ---
    # #############################################################
    (
        filtered_gmv,
        filtered_cogs,
    ...
    # #############################################################
    # --- 6. TAMPILKAN HEADER (LOGIKA DINAMIS BARU) ---
    # #############################################################

    # Mode 1: Jika kita upload file baru, gunakan info dari file itu
    ...
    # Mode 2: Jika kita pakai DB, ambil info dari data YANG SUDAH DIFILTER
    elif use_db == True and filtered_gmv is not None and not filtered_gmv.empty:
        # Ambil data header dari baris pertama data yang SUDAH DIFILTER
        company_name = filtered_gmv["Company"].iloc[0]
        branch_name_header = filtered_gmv["Branch"].iloc[0]

        # Buat periode data dinamis berdasarkan data yang difilter
        ...
        if min_date_str == max_date_str:
            period_str = min_date_str
        else:
            period_str = f"{min_date_str} s.d. {max_date_str}"

        st.title(f"Analisis Data: {company_name}")
        st.subheader(f"Cabang: {branch_name_header} | Periode Data: {period_str}")

    # Mode 3: Fallback (Data kosong atau filter tidak ada hasil)
    else:
